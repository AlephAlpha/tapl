...
impl DeBruijnTy {
    ...
    fn eqv(&self, other: &Self, ctx: &mut Context) -> bool {
        ...
        match (self_.as_ref(), other_.as_ref()) {
            ...
            (Self::Some(x, kn, ty), Self::Some(_, kn_, ty_)) => {
                kn == kn_ && ctx.with_name(x.clone(), |ctx| ty.eqv(ty_, ctx))
            }
            ...
            (Self::All(x, kn, ty), Self::All(_, kn_, ty_)) => {
                kn == kn_ && ctx.with_name(x.clone(), |ctx| ty.eqv(ty_, ctx))
            }
    ...
    pub fn kind_of(&self, ctx: &mut Context) -> Result<Rc<Kind>> {
        match self {
            ...
            Self::Abs(x, kn1, ty2) => {
                ctx.with_binding(x.clone(), Binding::TyVar(kn1.clone()), |ctx| {
...
impl DeBruijnTerm {
    ...
    pub fn eval1(&self, ctx: &Context, store: &mut Store) -> Result<Rc<Self>> {
        match self {
            ...
            Self::TApp(t, ty) => match t.as_ref() {
                ...
                _ => Ok(Self::t_app(t.eval1(ctx, store)?, ty.clone())),
            },
            Self::Ref(t) => {
                if t.is_val(ctx) {
            ...
            Self::Pack(ty1, t2, ty3) => {
                Ok(Self::pack(ty1.clone(), t2.eval1(ctx, store)?, ty3.clone()))
            }
    ...
    pub fn type_of(&self, ctx: &mut Context) -> Result<Rc<DeBruijnTy>> {
        match self {
            ...
            Self::TAbs(x, kn1, t2) => {
                ctx.with_binding(x.clone(), Binding::TyVar(kn1.clone()), |ctx| {
                    Ok(Ty::all(x.clone(), kn1.clone(), t2.type_of(ctx)?))
                })
            }
            Self::TApp(t1, ty2) => match t1.type_of(ctx)?.simplify(ctx).as_ref() {
                Ty::All(_, kn11, ty12) => {
                    if ty2.kind_of(ctx)? == *kn11 {
                        ty12.subst_top(ty2)
                    } else {
            ...
            Self::Assign(t1, t2) => match t1.type_of(ctx)?.simplify(ctx).as_ref() {
                Ty::Ref(ty) => {
                    if t2.type_of(ctx)?.eqv(ty, ctx) {
                        Ok(Ty::unit())
                    } else {
                        Err(Error::TypeError(
                    ...
                    match ty3.simplify(ctx).as_ref() {
                        Ty::Some(_, kn, ty) => {
                            if ty1.kind_of(ctx)? == *kn {
                                ...
                                Err(Error::TypeError(
                                    "type component does not have expected kind".to_string(),
                                ))
                            }
                        }
            ...
            Self::Unpack(x1, x2, t1, t2) => match t1.type_of(ctx)?.simplify(ctx).as_ref() {
                Ty::Some(_, kn, ty1) => {
                    ctx.with_binding(x1.clone(), Binding::TyVar(kn.clone()), |ctx| {
