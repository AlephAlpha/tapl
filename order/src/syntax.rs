...
pub const KEYWORDS: &[&str] = &[
    "true", "false", "if", "then", "else", "lambda", "split", "as", "in", "let", "ord", "lin",
    "un", "Bool",
];
...
impl BindingLinear for Binding {
    ...
    fn is_ord(&self) -> bool {
        matches!(self, Self::Var(ty) if ty.is_ord())
    }
}
        ...
        match self {
            ...
            Self::Let(x, t1, t2) => {
                ...
                Ok(Term::let_(
                    x_name.clone(),
                    t1,
                    ctx.with_name(x_name, |ctx| t2.to_named(ctx))?,
                ))
            }
        }
    }
