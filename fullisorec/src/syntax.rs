...
pub enum Ty<V = String> {
    Var(#[rc_term(into)] V),
    Rec(#[rc_term(into)] String, Rc<Self>),
    Float,
...
impl<V: Display> Display for Ty<V> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            Self::Rec(x, ty) => write!(f, "Rec {x}. {ty}"),
            t => t.fmt_arrow(f),
        }
    }
}
...
impl<V: Display> Term<V> {
    ...
    fn fmt_app(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Fold(ty) => write!(f, "fold [{ty}]"),
            Self::Unfold(ty) => write!(f, "unfold [{ty}]"),
            Self::Fix(t) => {
                write!(f, "fix ")?;
                t.fmt_atom(f)
            }
...
impl DeBruijnTy {
    fn map_vars_walk(
        ...
        Ok(match self {
            ...
            Self::Rec(x, ty) => Self::rec(x.clone(), ty.map_vars_walk(cutoff + 1, on_var)?),
...
impl DeBruijnTerm {
    fn map_vars_walk(
        ...
        Ok(match self {
            ...
            Self::Fold(ty) => Self::fold(on_type(cutoff, ty)?),
            Self::Unfold(ty) => Self::unfold(on_type(cutoff, ty)?),
        })
    }
...
impl Ty {
    pub fn to_de_bruijn(&self, ctx: &mut Context) -> Result<Rc<DeBruijnTy>> {
        match self {
            ...
            Self::Rec(x, ty) => Ok(DeBruijnTy::rec(
                x.clone(),
...
impl DeBruijnTerm {
    pub fn to_named(&self, ctx: &mut Context) -> Result<Rc<Term>> {
        match self {
            ...
            Self::Fold(ty) => Ok(Term::fold(ty.to_named(ctx)?)),
            Self::Unfold(ty) => Ok(Term::unfold(ty.to_named(ctx)?)),
        }
    }
}
...
impl Term {
    pub fn to_de_bruijn(&self, ctx: &mut Context) -> Result<Rc<DeBruijnTerm>> {
        match self {
            ...
            Self::Fold(ty) => Ok(DeBruijnTerm::fold(ty.to_de_bruijn(ctx)?)),
            Self::Unfold(ty) => Ok(DeBruijnTerm::unfold(ty.to_de_bruijn(ctx)?)),
        }
    }
}
