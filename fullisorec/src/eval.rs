...
impl DeBruijnTy {
    ...
    fn eqv(&self, other: &Self, ctx: &mut Context) -> bool {
        ...
        match (self_.as_ref(), other_.as_ref()) {
            ...
            (Self::Rec(x, ty), Self::Rec(_, ty_)) => ctx.with_name(x, |ctx| ty.eqv(ty_, ctx)),
            (Self::Var(i), _) => match ctx.get_binding_shifting(*i) {
                Ok(Binding::TyAbb(ty)) => ty.eqv(other, ctx),
                _ => match other {
                    Self::Var(j) => i == j,
                    _ => false,
                },
            ...
            (_, Self::Var(j)) => match ctx.get_binding_shifting(*j) {
                Ok(Binding::TyAbb(ty)) => self.eqv(&ty, ctx),
                _ => false,
            },
