...
pub enum Ty<V = String> {
    Var(#[rc_term(into)] V),
    String,
    Some(#[rc_term(into)] String, Rc<Self>),
    Arr(Rc<Self>, Rc<Self>),
    All(#[rc_term(into)] String, Rc<Self>),
...
pub enum Term<V = String> {
    ...
    App(Rc<Self>, Rc<Self>),
    TAbs(#[rc_term(into)] String, Rc<Self>),
...
impl<V: Display> Ty<V> {
    fn fmt_atom(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Some(x, ty) => write!(f, "{{Some {x}, {ty}}}"),
            Self::Record(fields) => {
                write!(f, "{{")?;
...
impl<V: Display> Display for Ty<V> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            Self::All(x, ty) => write!(f, "All {x}. {ty}"),
            _ => self.fmt_arrow(f),
        }
    }
}
...
impl DeBruijnTy {
    fn map_vars_walk(
        ...
        match self {
            ...
            Self::Some(x, ty) => Self::some(x.clone(), ty.map_vars_walk(cutoff + 1, on_var)),
            Self::Arr(t1, t2) => Self::arr(
                t1.map_vars_walk(cutoff, on_var),
                t2.map_vars_walk(cutoff, on_var),
            ),
            Self::All(x, ty) => Self::all(x.clone(), ty.map_vars_walk(cutoff + 1, on_var)),
...
impl DeBruijnTerm {
    fn map_vars_walk(
        ...
        match self {
            ...
            Self::TAbs(x, t) => {
                Self::t_abs(x.clone(), t.map_vars_walk(cutoff + 1, on_var, on_type))
            }
...
impl Ty {
    pub fn to_de_bruijn(&self, ctx: &mut Context) -> Result<Rc<DeBruijnTy>> {
        match self {
            ...
            Self::Some(x, ty) => Ok(DeBruijnTy::some(
                x.clone(),
            ...
            Self::All(x, ty) => Ok(DeBruijnTy::all(
                x.clone(),
                ctx.with_name(x.clone(), |ctx| ty.to_de_bruijn(ctx))?,
            )),
...
impl Term {
    pub fn to_de_bruijn(&self, ctx: &mut Context) -> Result<Rc<DeBruijnTerm>> {
        match self {
            ...
            Self::TAbs(x, t) => Ok(DeBruijnTerm::t_abs(
                x.clone(),
                ctx.with_name(x.clone(), |ctx| t.to_de_bruijn(ctx))?,
            )),
