...
pub const KEYWORDS: &[&str] = &[
    "true", "false", "if", "then", "else", "lambda", "succ", "pred", "iszero", "let", "in", "_",
    "Bool", "Nat",
];
    ...
    fn map_vars_walk(
        ...
        match self {
            ...
            Self::If(t1, t2, t3) => Ok(Self::if_(
                t1.map_vars_walk(cutoff, on_var)?,
                t2.map_vars_walk(cutoff, on_var)?,
                t3.map_vars_walk(cutoff, on_var)?,
            )),
            Self::Zero => Ok(Self::zero()),
            Self::Succ(t) => Ok(Self::succ(t.map_vars_walk(cutoff, on_var)?)),
            Self::Pred(t) => Ok(Self::pred(t.map_vars_walk(cutoff, on_var)?)),
            Self::IsZero(t) => Ok(Self::is_zero(t.map_vars_walk(cutoff, on_var)?)),
            Self::Let(x, t1, t2) => Ok(Self::let_(
                x.clone(),
                t1.map_vars_walk(cutoff, on_var)?,
                t2.map_vars_walk(cutoff + 1, on_var)?,
            )),
        }
    }
