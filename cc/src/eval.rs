...
impl DeBruijnTy {
    fn compute(&self, ctx: &Context) -> Result<Rc<Self>> {
        match self {
            Self::App(ty, t) => match ty.as_ref() {
                Self::Prf => {
                    if let Term::All(x, ty_, t_) = t.whnf(ctx).as_ref() {
                        Ok(Self::pi(
                            x.clone(),
                            ty_.clone(),
                            Self::app(Self::prf(), t_.clone()),
                        ))
                    } else {
    ...
    fn eqv(&self, other: &Self, ctx: &mut Context) -> bool {
        ...
        match (self_.as_ref(), other_.as_ref()) {
            ...
            (Self::Prop, Self::Prop) | (Self::Prf, Self::Prf) => true,
            _ => false,
        }
    }
    ...
    pub fn kind_of(&self, ctx: &mut Context) -> Result<Rc<DeBruijnKind>> {
        match self {
            ...
            Self::Prop => Ok(Kind::star()),
            Self::Prf => Ok(Kind::pi("_".to_string(), Self::prop(), Kind::star())),
        }
    }
}
...
impl DeBruijnTerm {
    ...
    fn eqv_wh(&self, other: &Self, ctx: &mut Context) -> bool {
        match (self, other) {
            ...
            (Self::All(x, ty1, t1), Self::All(_, ty1_, t1_)) => {
                ty1.eqv(ty1_, ctx)
                    && ctx
                        .with_binding(x.clone(), Binding::Var(ty1.clone()), |ctx| t1.eqv(t1_, ctx))
            }
    ...
    pub fn type_of(&self, ctx: &mut Context) -> Result<Rc<DeBruijnTy>> {
        match self {
            ...
            Self::All(x, ty, t) => {
                if matches!(ty.kind_of(ctx)?.as_ref(), Kind::Star) {
                    ctx.with_binding(x.clone(), Binding::Var(ty.clone()), |ctx| {
                        if t.type_of(ctx)?.eqv(&Ty::Prop, ctx) {
                            Ok(Ty::prop())
                        } else {
                            Err(Error::TypeError("Prop type expected".to_string()))
                        }
                    })
                } else {
                    Err(Error::KindError("Star kind expected".to_string()))
