...
impl DeBruijnTy {
    fn map_vars_walk(
        ...
        match self {
            ...
            Self::Rec(x, ty) => Ok(Self::rec(x.clone(), ty.map_vars_walk(cutoff + 1, on_var)?)),
            ...
            Self::Variant(fields) => Ok(Self::variant(
                fields
                    .iter()
                    .map(|(label, ty)| Ok((label.clone(), ty.map_vars_walk(cutoff, on_var)?)))
                    .collect::<Result<Vec<_>>>()?,
...
impl DeBruijnTerm {
    fn map_vars_walk(
        ...
        match self {
            ...
            Self::Case(t, cases) => Ok(Self::case(
                t.map_vars_walk(cutoff, on_var, on_type)?,
                cases
                    .iter()
                    .map(|(l, x, t)| {
